...
// } 
   ...
   async function getAllPayedInstallment(id,na,em,amount,famount,install,monthlyInt){
                  ...
                  if(querySnapshot.empty){
                    console.log(id+'-----------------------emty mare---'+em);
                    data.push({id:data.length,name:na,email:em,date:id,amount:amount,OverallAmount:famount,MonthlyInstallemt:install,MonthlyInterest:monthlyInt,Remaing_Amount:famount});

                  }
                  querySnapshot.forEach((doc) => {
                    var m=parseInt(doc.id.slice(5, 7)) ;
                    var d=parseInt(doc.id.slice(8,10)) ;
                    var y=(parseInt(doc.id));
                     var full=(y+'-'+m+'-'+d);
                     console.log(full); 
...
var stdNo=0;
 ...
 function AddItemToTable(Name,Email,Date,Amount,oveall,monthly,intres,reaming){
  ...
  tBox.setAttribute('Class', 'check');
        ...
        cell10.appendChild(tBox);
   
     
 }


//  if(arr.length==data.length && starting==true){
//   console.log('zb ');
         ...
         for(var i=0;i<myElements.length;i++){
              if(myElements[i].checked==true){
                console.log(table.rows[i+1].cells[2].innerHTML);
                    AddInstallment(table.rows[i+1].cells[2].innerHTML,table.rows[i+1].cells[3].innerHTML,table.rows[i+1].cells[7].innerHTML);
              }
          }
      }
...
///////////////////////////////////////////////////
  function AddInstallment(ooo,id,amount){
...
export default memo (AddInstallment);
