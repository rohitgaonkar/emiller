...
function MemberViewLoan() {
 ...
 window.onload =getdata();
  ...
  async function getdata(){
    ...
    const querySnapshot =await getDocs(collection(db,`Admin/${oo}/${ll}/${kk}/Members/${tt}/Loan`));
 ...
 var n=0;
     querySnapshot.forEach(doc=>{
      ...
      async function giveMeLoan(id){
        ...
        const hot =await getDocs(collection(db,`/Admin/${oo}/${ll}/${kk}/Members/${tt}/Loan/${id}/Installment`));
              hot.forEach(doc=>{
              
                Members.push({month:(doc.id),amount:(doc.data().Amount)});
